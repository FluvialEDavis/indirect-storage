...
```{r direct_storage}
q <- daily_upper_middle$mean_daily_q
...
```{r cumulative-plot, message=F, warning=F}
direct_indirect %>%
  gather(category, measurement, mean_daily_prcp:total) %>%
  filter(category %in% c("cum_p", "cum_q", 
                         "d_s", "i_s", "total")) %>%
