...
daily_upper_middle <- upper_middle %>%
  ...
  summarise(mean_daily_prcp = sum(precip_mm),
            ...
            mean_daily_q_mm = mean_daily_q / (7.7e05) * 1000 * 86400,
...
hourly_upper_middle <- upper_middle %>%
  ...
  group_by(dt_hour) %>%
  summarise(hourly_q_mm = mean(q_est, 
                            na.rm = TRUE) / (7.7e05) * 1000 * 3600,
            hourly_p = sum(precip_mm, na.rm = TRUE),
            hourly_t = mean(air_temp, na.rm = TRUE)) %>%
  mutate(row_id = seq(1, 3322)) %>%
  tq_mutate(select = hourly_q_mm,   # This function populates a 72-hr moving avg
            mutate_fun = rollapply,
            width = 72,
            align = "right",
            FUN = mean,
            na.rm = TRUE,
            col_rename = "roll_mean_q")
```
...
hourly_upper_middle %>%
  ...
  geom_line(aes(y = roll_mean_q), color = "darkred") +
...
recess_extract <- function(q, p, start_row = 1) {
  ...
  while(i < nrow(hourly_upper_middle) - 1){
    ...
    if(mean(hourly_upper_middle$hourly_p[c(i - 24, i)]) < 0.002 &
       hourly_upper_middle$roll_mean_q[i] <
       hourly_upper_middle$roll_mean_q[i - 1]) {
...
recess_extract(hourly_upper_middle$hourly_q_mm, hourly_upper_middle$hourly_q_mm, start_row = 73)
```
...
recession_limbs <- recession_limbs %>%
  ...
  select(dt_hour, q = roll_mean_q, group, row_id)
```
...
```{r variable-timestep}
threshold <- mean(hourly_upper_middle$roll_mean_q, na.rm = TRUE) * 0.001
